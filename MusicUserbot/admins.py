...
async def skip(client, m: Message):
    ...
    if len(m.command) < 2:
        ...
        if op == 0:
            await m.reply("**❌ There was nothing in the queue to skip!**")
        elif op == 1:
            await m.reply("*Empty Queue, Left Voice Chat*")
        else:
            await m.reply(
...
async def pause(client, m: Message):
    ...
    if chat_id in QUEUE:
        try:
            ...
            await m.reply(
                f"**⏸ Playback is paused.**\n\n• To resume playback, use command » {HNDLR}resume"
            )
        except Exception as e:
            await m.reply(f"**ERROR** \n`{e}`")
    else:
